...
try {
    $query = "SELECT * FROM
              ...
              ORDER BY borrowed_datetime DESC, returned_datetime DESC";
    $logs = $pdo->prepare($query);
    $logs->bindValue(':borrowed_datetime', "%$date%", PDO::PARAM_STR);
    $logs->bindValue(':returned_datetime', "%$date%", PDO::PARAM_STR);
    $logs->execute();
